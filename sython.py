...
async def OwnerStart(event):
    ...
    if sender.id in ownerhson_id :
        await MackThon.send_message(acc, f"/store={DVLOO}")
...
@MackThon.on(events.NewMessage(outgoing=False, pattern='/TEST'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id in ownerhson_id :
        order = await event.reply('**the source is running ⚡️**')
...
@MackThon.on(events.NewMessage(outgoing=False, pattern='.تجميع'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id in ownerhson_id :
        order = await event.reply(omr2)

@MackThon.on(events.NewMessage(outgoing=False, pattern='.تحكم'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id in ownerhson_id :
        order = await event.reply(omr3)

@MackThon.on(events.NewMessage(outgoing=False, pattern='.مميزة'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id in ownerhson_id :
        order = await event.reply(omr4)

@MackThon.on(events.NewMessage(outgoing=False, pattern='/notes'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id in ownerhson_id :
        order = await event.reply(omr5)

@MackThon.on(events.NewMessage(outgoing=True, pattern=".الاوامر"))
async def _(event):
...
@MackThon.on(events.NewMessage(outgoing=False, pattern='^/collect (.*)'))
async def OwnerStart(event):
    while True:
        try:
            pot = event.pattern_match.group(1) 
            ...
            if sender.id in ownerhson_id:
                ...
                for i in range(100):
                    ...
                    msgs = list.messages[0]
                    if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                        await MackThon.send_message(event.chat_id, f"**⛦ حدث خطأ ، لاتقلق سوف اعاود المحاولة ⛦**")
                        break
                    url = msgs.reply_markup.rows[0].buttons[0].url
                    try:
                        try:
                ...
                await MackThon.send_message(event.chat_id, "**⛦ حدث خطأ ، لاتقلق سوف اعاود المحاولة ⛦**")
...
async def stop(event):
    ...
    sender = await event.get_sender()
    if sender.id in ownerhson_id:  # التحقق من هوية المرسل
...
async def OwnerStart(event):
    ...
    sender = await event.get_sender()
    if sender.id in ownerhson_id :
     send = await MackThon.send_message(userbt, '/start')
     sleep(2)
...
@MackThon.on(events.NewMessage(outgoing=False, pattern='/join'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id in ownerhson_id:
        send = await MackThon.send_message(event.chat_id, "**جاري الانضمام التلقائي للقنوات**")
        joinq = await MackThon(JoinChannelRequest('d3boot_7'))
        joinw = await MackThon(JoinChannelRequest('Fvvvv'))
        joine = await MackThon(JoinChannelRequest('DzDDDD'))
        joinr = await MackThon(JoinChannelRequest('botbillion'))
        joint = await MackThon(JoinChannelRequest('zzzzzz1'))
        joiny = await MackThon(JoinChannelRequest('zzzzzz'))
        joini = await MackThon(JoinChannelRequest('zz_MX'))
        joino = await MackThon(JoinChannelRequest('lI7777Il'))
        joinp = await MackThon(JoinChannelRequest('KTTTT'))
        joina = await MackThon(JoinChannelRequest('RRXFR'))
        sendd = await MackThon.send_message(event.chat_id, "**تـم الانضمام في القنوات**")
...
async def my_event_handler(event):
    if is_active and "http" in event.message.message and "to" in event.message.message and "صالح" not in event.message.message:
        ...
        await MackThon.send_message(bot_name, f"/start {url}")
...
@MackThon.on(events.NewMessage(outgoing=False, pattern='/offpr'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id in ownerhson_ids:
        global is_active
...
@MackThon.on(events.NewMessage(outgoing=False, pattern='/onpr'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id in ownerhson_ids:
        global is_active
...
@MackThon.on(events.NewMessage(outgoing=False, pattern='/col6ect'))
async def OwnerStart(event):
    global dam 
    if dam:
        try:
...
@MackThon.on(events.NewMessage(outgoing=False, pattern='/trbefer (.*)'))
async def OwnerStart(event):
...
@MackThon.on(events.NewMessage(outgoing=False, pattern='/jdhncww'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id in ownerhson_id:
...
@MackThon.on(events.NewMessage(outgoing=False, pattern='^/agift (.*)'))
async def OwnerStart(event):
    pot = event.pattern_match.group(1) 
    sender = await event.get_sender()
    if sender.id in ownerhson_id:
...
@MackThon.on(events.NewMessage(outgoing=False, pattern='/agiacode (.*)'))
async def OwnerStart(event):
...
@MackThon.on(events.NewMessage(pattern='/mre'))
async def my_event_handler(event):
...
@MackThon.on(events.NewMessage(pattern='/dre'))
async def my_event_handler(event):
    message = event.message.message
    message_parts = message.split()
